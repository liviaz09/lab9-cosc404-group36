...
public class Snowflake {
	...
	public ResultSet query2() throws SQLException {
		...
		return stmt.executeQuery("SELECT EMPLOYEENAME, COUNT(DISTINCT ORDERID) as NUMORDERS, SUM(PRICE*QUANTITY) as TOTALORDERVALUE "
		+"FROM (LAB.PUBLIC.ORDERS NATURAL JOIN LAB.PUBLIC.EMPLOYEE) NATURAL JOIN (LAB.PUBLIC.ORDEREDPRODUCT NATURAL JOIN LAB.PUBLIC.PRODUCT) NATURAL JOIN LAB.PUBLIC.CUSTOMER"
		 +" WHERE LISTPRICE BETWEEN 0.5 AND 100 AND (CUSTOMERNAME LIKE '%e%'  OR CUSTOMERNAME LIKE '%E%')"
		+"GROUP BY EMPLOYEENAME ORDER BY TOTALORDERVALUE DESC "
		+"LIMIT 5;");
	}
